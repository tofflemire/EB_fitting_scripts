...
def binary_lc(t,t0,P,rsrp,aorpprs,inc,sbr,ecc,omega,g1,g2):
...
def log_likelihood(params, t_lc, f, ferr, lc_id, t_rv, rv, rverr, rv_id):
    ...
    t0,P,rsrp,aorpprs,cosi,sqesinw,sqecosw,g1_f1,g2_f1,sbr_f1,logf_lc_f1, g1_f2,g2_f2,sbr_f2,logf_lc_f2, g1_f3,g2_f3,sbr_f3,logf_lc_f3, K1pK2,q,gamma,mu = params
    ...
    params_inlc_f1 = t0,P,rsrp,aorpprs,inc,sbr_f1,ecc,omega,g1_f1,g2_f1
    params_inlc_f2 = t0,P,rsrp,aorpprs,inc,sbr_f2,ecc,omega,g1_f2,g2_f2
    params_inlc_f3 = t0,P,rsrp,aorpprs,inc,sbr_f3,ecc,omega,g1_f3,g2_f3
...
def log_prior(params):#, kde_rr_sbr):
    t0,P,rsrp,aorpprs,cosi,sqesinw,sqecosw, g1_f1,g2_f1,sbr_f1,logf_lc_f1, g1_f2,g2_f2,sbr_f2,logf_lc_f2, g1_f3,g2_f3,sbr_f3,logf_lc_f3, K1pK2,q,gamma,mu = params
    ...
    ##FILTER 1
    ##proper bounds given model
    #q1_f1 = np.sqrt(g1_f1)/(1.0+((0.5-g2_f1)/g2_f1))
    #    
    #q1_mu_f1 = q1_val_1
    #q1_sig_f1 = qerr_1
    #q1_prior_f1 = np.log(1.0/(np.sqrt(2*np.pi)*q1_sig_f1))-0.5*(q1_f1-q1_mu_f1)**2/q1_sig_f1**2 #log_of_normal(q1,q1_mu,q1_sig)
    #    
    ##proper bounds given model
    #q2_f1 = np.sqrt(g1_f1) - q1_f1
    #    
    #q2_mu_f1 = q2_val_1
    #q2_sig_f1 = qerr_1
    #q2_prior_f1 = np.log(1.0/(np.sqrt(2*np.pi)*q2_sig_f1))-0.5*(q2_f1-q2_mu_f1)**2/q2_sig_f1**2 #log_of_normal(q2,q2_mu,q2_sig)
    #    
    ##FILTER 2
    ##proper bounds given model
    #q1_f2 = np.sqrt(g1_f2)/(1.0+((0.5-g2_f2)/g2_f2))
    #    
    #q1_mu_f2 = q1_val_2
    #q1_sig_f2 = qerr_2
    #q1_prior_f2 = np.log(1.0/(np.sqrt(2*np.pi)*q1_sig_f2))-0.5*(q1_f2-q1_mu_f2)**2/q1_sig_f2**2 #log_of_normal(q1,q1_mu,q1_sig)
    #    
    ##proper bounds given model
    #q2_f2 = np.sqrt(g1_f2) - q1_f2
    #    
    #q2_mu_f2 = q2_val_2
    #q2_sig_f2 = qerr_2
    #q2_prior_f2 = np.log(1.0/(np.sqrt(2*np.pi)*q2_sig_f2))-0.5*(q2_f2-q2_mu_f2)**2/q2_sig_f2**2 #log_of_normal(q2,q2_mu,q2_sig)
    #    
    ##FILTER 3
    ##proper bounds given model
    #q1_f3 = np.sqrt(g1_f3)/(1.0+((0.5-g2_f3)/g2_f3))
    #        
    #q1_mu_f3 = q1_val_3
    #q1_sig_f3 = qerr_3
    #q1_prior_f3 = np.log(1.0/(np.sqrt(2*np.pi)*q1_sig_f3))-0.5*(q1_f3-q1_mu_f3)**2/q1_sig_f3**2 #log_of_normal(q1,q1_mu,q1_sig)
    #    
    ##proper bounds given model
    #q2_f3 = np.sqrt(g1_f3) - q1_f3
    #    
    #q2_mu_f3 = q2_val_3
    #q2_sig_f3 = qerr_3
    #q2_prior_f3 = np.log(1.0/(np.sqrt(2*np.pi)*q2_sig_f3))-0.5*(q2_f3-q2_mu_f3)**2/q2_sig_f3**2 #log_of_normal(q2,q2_mu,q2_sig)

    #SALT_HARPS offset
    #mu_mu = 0.36
    #mu_sig = 0.22
    #mu_prior = np.log(1.0/(np.sqrt(2*np.pi)*mu_sig))-0.5*(mu-mu_mu)**2/mu_sig**2 #log_of_normal(q2,q2_mu,q2_sig)

    #from Spectra/BF
    #Fit will fill this prior, but it is well constrained
    #fratio_mu = 1.0
    #fratio_sig = 0.027
    #fratio_prior = log_of_normal(fratio,fratio_mu,fratio_sig)
    #moved to a joint prior below
...
#FILTER 1
#T1,T2,R1,R2,SBR,f = np.loadtxt(str(kde_file_1),unpack=True,delimiter=',')
#x = R2/R1
#y = SBR
#kde_rr_sbr_f1 = kde.gaussian_kde([x,y])
kde_rr_sbr_f1 = pkl.load(open(str(kde_file_1),'rb'))

#FILTER 2
#T1,T2,R1,R2,SBR,f = np.loadtxt(str(kde_file_2),unpack=True,delimiter=',')
#x = R2/R1
#y = SBR
#kde_rr_sbr_f2 = kde.gaussian_kde([x,y])
kde_rr_sbr_f2 = pkl.load(open(str(kde_file_2),'rb'))

#FILTER 3
#T1,T2,R1,R2,SBR,f = np.loadtxt(str(kde_file_3),unpack=True,delimiter=',')
#x = R2/R1
#y = SBR
#kde_rr_sbr_f3 = kde.gaussian_kde([x,y])
...
#Labels
labels = ['t0','P','rsrp','aorpprs','cosi','sqesinw','sqecosw',  'g1_f1','g2_f1','sbr_f1','logf_lc_f1',  'g1_f2','g2_f2','sbr_f2','logf_lc_f2',  'g1_f3','g2_f3','sbr_f3','logf_lc_f3',  'K1+K2','q','gamma','mu']
...
if continue_run == False:
	...
	pos = mcmc_start + np.array([1e-4,1e-6,1e-4,1e-4,1e-4,1e-4,1e-4, 1e-4,1e-4,1e-4,1e-4, 1e-4,1e-4,1e-4,1e-4, 1e-4,1e-4,1e-4,1e-4, 1e-4,1e-4,1e-4,1e-4])*np.random.randn(mcmc_start.size*5, mcmc_start.size)
